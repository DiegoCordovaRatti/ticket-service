...
export default function TicketDescription(props) {
  const {address, assignedDate, assignedTime, clientName, createdBy, createdOn, 
    flatNumber, regionComune, reqType, subject, subjectDetails, techDepartment, id } = props.ticket
    
  return (
    <div>
      <Descriptions title={`Requerimiento`} bordered column={{xs: 1, sm: 1, md: 1, lg:2}} size='small'>
        <Descriptions.Item label={<Text strong>ID:</Text>}>{ id } </Descriptions.Item>
        ...
        <Descriptions.Item label={<Text strong>Dirección & departamento</Text>} >{ address } { flatNumber === '' ?
          flatNumber : `, ${flatNumber}`}
        </Descriptions.Item>
        <Descriptions.Item label={<Text strong>Región / Comuna:</Text>}>{ regionComune[0] } | {regionComune[1]}
        </Descriptions.Item>
        <Descriptions.Item label={<Text strong>Fecha y hora agendada</Text>}>{ assignedDate } <br />{assignedTime}
        </Descriptions.Item>
        ...
        <Descriptions.Item label={<Text strong>Detalles</Text>} span={2}>{ subjectDetails }</Descriptions.Item>
      </Descriptions>
